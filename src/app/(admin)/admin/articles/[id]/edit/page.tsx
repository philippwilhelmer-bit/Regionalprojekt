  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-4 mb-4">
        ...
        <h1 className="text-2xl font-bold text-ink font-headline">Artikel bearbeiten</h1>
        {articleUrl && (
          <a
            href={articleUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="ml-auto text-sm text-slate hover:text-slate/80 flex items-center gap-1"
          >
            Artikel ansehen
      ...
      {article.status === 'PUBLISHED' && articleUrl && (
        <div className="bg-surface rounded-sm p-4 mb-4">
          <p className="text-xs font-medium text-ink-dim uppercase tracking-wide mb-3">Vorschau</p>
          <div className="flex gap-4">
            ...
            <div className="min-w-0">
              <h2 className="text-sm font-semibold text-ink line-clamp-2">{article.title}</h2>
              {article.metaDescription && (
                <p className="text-xs text-ink-dim mt-1 line-clamp-2">{article.metaDescription}</p>
              )}
              <div className="flex items-center gap-2 mt-2">
                {article.bezirke.map((ab) => (
                  <span key={ab.bezirkId} className="text-[10px] px-1.5 py-0.5 rounded bg-parchment-dim text-ink-muted">
                    {ab.bezirk.name}
                  </span>
                ))}
              </div>
