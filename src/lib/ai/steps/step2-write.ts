...
// System prompt kept as a module-level constant — iterable without code changes (RESEARCH.md pitfall 5).
// NOTE: LOW confidence until validated against real OTS data in Phase 7.
const SYSTEM_PROMPT_TEMPLATE = `Du bist ein österreichischer Lokaljournalist. Schreibe den folgenden Presseartikel als kurze, sachliche Nachricht auf Hochdeutsch um.
...
export async function runStep2Write(
  ...
  const systemPrompt = SYSTEM_PROMPT_TEMPLATE.replace('{bezirkContext}', bezirkContext)
