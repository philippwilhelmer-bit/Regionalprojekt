...
// Lazy import — will fail until step1-tag.ts is created (RED phase)
...
function makeMockClient(
  ...
  const defaultResponse = {
    content: [
      {
        type: 'text' as const,
        text: JSON.stringify({ bezirkSlugs: ['liezen'], hasNamedPerson: false }),
      },
    ],
  ...
  it('returns hasNamedPerson boolean from structured JSON response', async () => {
    const client = makeMockClient({
      content: [
        {
          type: 'text',
          text: JSON.stringify({ bezirkSlugs: ['liezen'], hasNamedPerson: true }),
        },
      ],
    })
  ...
  it('handles empty bezirkSlugs (no geographic match)', async () => {
    const client = makeMockClient({
      content: [
        {
          type: 'text',
          text: JSON.stringify({ bezirkSlugs: [], hasNamedPerson: false }),
        },
      ],
    })
